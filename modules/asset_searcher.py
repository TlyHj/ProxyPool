...
class AssetSearcher:
    ...
    @staticmethod
    def _resolve_secret(value, env_name):
        """优先使用界面填写的值；为空时从环境变量读取，避免必须明文落盘。"""
        value = (value or '').strip()
        return value or os.environ.get(env_name, '').strip()

    @staticmethod
    def _safe_request_error(exc):
        """避免请求异常日志泄露包含 api-key/key 参数的完整 URL。"""
        text = str(exc)
        try:
            ...
            if url:
                ...
                for k, v in urllib.parse.parse_qsl(parsed.query, keep_blank_values=True):
                    if k.lower() in {'key', 'api-key', 'email', 'x-quaketoken'}:
                        redacted_query.append((k, '***'))
                    else:
                        redacted_query.append((k, v))
                safe_url = urllib.parse.urlunsplit((
                    parsed.scheme, parsed.netloc, parsed.path,
                    urllib.parse.urlencode(redacted_query), parsed.fragment
                ))
    ...
    def _search_fofa(self, key, query, size):
        ...
        key = self._resolve_secret(key, 'PROXYPOOL_FOFA_KEY')
        if not key:
            self.log("[!] (Fofa) 失败: 未提供API Key，可在设置中填写或设置 PROXYPOOL_FOFA_KEY。")
            return []
        ...
        try:
            ...
            self.log(f"[!] (Fofa) 请求失败: {self._safe_request_error(e)}")
    ...
    def _search_quake(self, key, query, size):
        ...
        key = self._resolve_secret(key, 'PROXYPOOL_QUAKE_KEY')
        if not key:
            self.log("[!] (Quake) 失败: 未提供API Key，可在设置中填写或设置 PROXYPOOL_QUAKE_KEY。")
            return []
        ...
        try:
            ...
            self.log(f"[!] (Quake) 请求失败: {self._safe_request_error(e)}")
        except Exception as e:
    ...
    def _search_hunter(self, key, query, size):
        ...
        key = self._resolve_secret(key, 'PROXYPOOL_HUNTER_KEY')
        if not key:
            self.log("[!] (Hunter) 失败: 未提供API Key，可在设置中填写或设置 PROXYPOOL_HUNTER_KEY。")
            return []
        ...
        try:
            ...
            self.log(f"[!] (Hunter) 请求失败: {self._safe_request_error(e)}")
        except Exception as e:
