...
class ProxyServer:
    ...
    def _get_upstream_connection(self, target_host, target_port):
        ...
        try:
            ...
            failed_proxy = self._rotator.report_failure(addr)
            if failed_proxy:
                failures = failed_proxy.get('consecutive_failures', 0)
                self.log(f"[!] 已临时标记上游代理不可用: {addr} (失败 {failures} 次)")
                if self.on_proxy_failed:
                    try:
                        self.on_proxy_failed(addr)
                    except Exception:
                        pass
            remote_socket.close()
            return None
